...
//        re "My public IP address is: " . $ip;
        ...
        if (request('myCourses') !== null) {
            ...
            $data = Courses::leftJoin('user_courses', 'course_id', 'courses.id')->where('user_id', $currentUser->id)->withCount('users')->get();
        }
        if (request('search') !== null) {
            $title = "نتائج البحث";
            $data = Courses::where('name', 'like', '%' . request('search') . '%')->withCount('users')->get();
//            echo request('search');
//            return $data;

        }
        if (request('search') == null && request('myCourses') == null) {
            $data = Courses::withCount('users')->get();
        }
...
//        return view('course-view', compact('data'));
        ...
        $currentUser = Auth::user();
        $course = Courses::where('id', $course_id)->with('section')->first();
